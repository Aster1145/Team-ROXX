...
export default function MentorsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const mentorsTableData = (mentorsRes.data as Profile[]) || [];

      // Combine dedicated mentors table rows with profile mentors
      const mentorsMap = new Map<string, Profile>();

      mentorsTableData.forEach((m) => {
        mentorsMap.set(m.id || m.email, {
          ...m,
      ...
      allProfiles
        .filter((p) => p.role === "mentor" && p.department !== "Trainee")
        .forEach((m) => {
          if (!mentorsMap.has(m.id) && !mentorsMap.has(m.email)) {
            mentorsMap.set(m.id || m.email, m);
          }
        });

      setMentors(Array.from(mentorsMap.values()));
  ...
  const handleRemoveMentor = async (id: string, name: string) => {
    ...
    try {
      await Promise.all([
        ...
        supabase.from("mentors").delete().eq("id", id),
      ]);
  ...
  // Filter visible mentors list: if logged in as mentor, show their profile or directory
  const displayedMentors = isUserMentor && mentorProjectId
    ? mentors.filter((m) => m.id === profile?.id || m.project_id === mentorProjectId)
    : mentors;
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">
            Faculty advisors, industry specialists, and academic mentors guiding specific team projects.
          </p>
        </div>
        {userIsCaptain && (
              ...
              <CardContent className="p-4 sm:p-6 space-y-4">
                ...
                <div className="rounded-xl border border-slate-200/80 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/50 p-3.5 space-y-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    {assignedProject ? (
                      <Badge variant="forest" className="font-semibold text-xs bg-emerald-100 text-emerald-900 border-emerald-300 dark:bg-emerald-950 dark:text-emerald-200">
                  ...
                  {assignedProject && (
                    <div className="pt-2 border-t border-slate-200/60 dark:border-slate-700/60">
                      <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1.5 mb-2">
                        <Users className="h-3.5 w-3.5 text-blue-500 shrink-0" /> Assigned Project Students ({projectMembers.length}):
                      </p>
        ...
        {displayedMentors.length === 0 && (
          <Card className="border-dashed border-2 border-slate-300 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/40 p-8 text-center">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-md mx-auto">
              Add faculty advisors or academic mentors to guide team projects and evaluate student work.
            </p>
            {userIsCaptain && (
